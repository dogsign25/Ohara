  ...
  return (
    <div className="relative w-full max-w-sm">
      ...
      {open && results.length > 0 && (
        <ul className="absolute top-full mt-1 w-full bg-gray-900 border border-white/10 rounded-xl overflow-hidden shadow-2xl z-50">
          {results.map(node => (
            <li
            ...
            >
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${TYPE_STYLE[node.type] ?? ''}`}>
                {node.type}
              </span>
            </li>
          ))}
